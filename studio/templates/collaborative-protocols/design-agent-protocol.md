# Collaborative Protocol for Design Roles
...
1. **Ask clarifying questions:**
   ...
   - *Use `structured choice capture` to batch up to 4 constrained questions at once*
...
2. **Present 2-4 options with reasoning:**
   ...
   - *After the full explanation, use `structured choice capture` to capture the decision*
...
Use the `structured choice capture` tool to present decisions as a selectable UI instead of
...
2. **Capture the decision** — Call `structured choice capture` with concise option labels
...
- Batch up to 4 independent questions in a single `structured choice capture` call
...
- When running as a delegated worker (tool may not be available) — structure your
  text output so the orchestrator can present options via structured choice capture
...
**Example — multi-question batch for clarifying questions:**
  ...
  structured choice capture with questions:
