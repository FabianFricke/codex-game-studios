...
This workflow is the entry point for new users. It does NOT assume you have a game
...
Check:
- **Engine configured?** Read `studio/docs/technical-preferences.md`. If the
...
> **Welcome to coding-agent environment Game Studios!**
>
...
to take first. Never auto-run the next workflow.
...
When the user chooses their next step, let them trigger the workflow themselves
or offer to run it for them. Either way, the `start` workflow's job is done
...
This workflow follows the collaborative design principle:
...
4. **No auto-execution** — recommend the next workflow, don't run it without asking
