...
When this workflow runs, orchestrate the audio team through a structured pipeline.

**Decision Points:** At each step transition, present
the user with the contributing roles' proposals as selectable options. Write the
...
Use the delegation step to assign each team member a focused workstream:
...
Always provide full context in each delegated brief (feature description, existing audio assets, design doc references).

3. **Orchestrate the audio team** in sequence:

### Step 1: Audio Direction (audio-director)
Delegate to `audio-director` to:
...
Delegate to `technical-artist` to:
...
Delegate to `gameplay-programmer` to:
