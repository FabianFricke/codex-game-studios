...
This workflow validates whether the project is ready to advance to the next development
phase. It checks for required artifacts, quality standards, and blockers.

**Distinct from `project-stage-detect`**: That workflow is diagnostic ("where are we?").
This workflow is prescriptive ("are we ready to advance?" with a formal verdict).
...
**Date**: [date]
**Checked by**: gate-check workflow
