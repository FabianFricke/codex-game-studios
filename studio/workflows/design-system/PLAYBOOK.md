...
> Run `map-systems` first to create the systems index, then use this workflow
...
Read all relevant context **before** asking the user anything. This workflow's
...
Use the template structure from `studio/templates/game-design-document.md`:
...
2. **Questions**: Ask clarifying questions specific to this section. Use
   structured questions for constrained prompts, conversational text for open-ended
   exploration.
...
**Agent delegation**: For complex mechanics, use the delegation step to delegate to
...
delegate to `systems-designer` via the delegation step. Provide:
...
If yes, invoke the design-review workflow on the completed file.
...
This workflow delegates to specialist roles for domain expertise. The main session
...
| Progression, XP, unlocks | `game-designer` | `systems-designer` (curves), `economy-designer` (sinks) |
...
**When delegating via delegation step**:
...
- The main session presents the agent's output to the user as a structured question
...
This workflow follows the collaborative design principle at every step:

1. **Question -> Options -> Decision -> Draft -> Approval** for every section
2. Ask the user a structured question at every decision point (Explain -> Capture pattern):
