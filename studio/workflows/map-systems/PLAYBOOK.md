...
workflow — it requires human judgment because concept docs rarely enumerate every
system explicitly.
...
- "Progression" implies: XP system, level-up mechanics, unlock tree, unlock
...
Then ask the user to capture feedback:
...
Using the template at `studio/templates/systems-index.md`, populate the
...
- The user runs `map-systems [system-name]`
- The user runs `map-systems next`
...
Ask the user: "Start designing [system-name] now, pick a different
system, or stop here?"

### Step 6b: Hand Off to `design-system`

Once a system is selected, run the `design-system` workflow for `[system-name]`.

The `design-system` workflow handles the full GDD authoring process:
...
**Do not duplicate the `design-system` workflow here.** This workflow owns the systems
...
This workflow follows the collaborative design principle at every phase:
